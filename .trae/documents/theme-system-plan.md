...
### 阶段2: 创建主题样式系统
4. **扩展 Tailwind 配置** (`tailwind.config.js`)
   - 添加主题相关的颜色扩展
   - 添加自定义圆角、阴影
   - 配置字体族

5. **创建主题CSS变量文件** (`src/styles/themes.css`)
   - 为每个主题定义CSS变量
   - 动态主题切换的变量映射

6. **创建主题包装组件** (`src/components/ThemeProvider.vue`)
   - 监听主题变化
   - 动态应用CSS类
   - 字体加载管理

### 阶段3: 创建主题化组件
7. **创建主题化Header组件** (`src/components/themes/ThemeHeader.vue`)
   - 支持6种风格的Header
   - 根据当前主题渲染不同样式

8. **创建主题化FeatureTiles组件** (`src/components/themes/ThemeFeatureTiles.vue`)
   - Bento网格布局适配各主题
   - 卡片样式主题化

9. **创建主题化Footer组件** (`src/components/themes/ThemeFooter.vue`)
   - 页脚样式主题化

### 阶段4: 创建设置面板
10. **创建设置弹窗组件** (`src/components/SettingsModal.vue`)
    - 主题选择器UI
    - 主题预览缩略图
    - 设置持久化
